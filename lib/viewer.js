...
// actually canvg is optional
//var canvg = require("canvg");

var Class = require('js-class');
gFeatureArray = [];

// jQuery.extend would work too

var FeatureViewer =  Class({
/** @lends Biojs.FeatureViewer */
  ...
  constructor: function(options) {
    ...
    gFeatureArray.push(this);
    this.instance = gFeatureArray.length-1;
    gFeatureArray[this.instance].opt.json = options.json || {};

    gFeatureArray[this.instance].initRaphael();

    console.log(this);

    console.log(gFeatureArray[this.instance].opt.json);

    if (Object.keys(gFeatureArray[this.instance].opt.json).length > 0) {
      gFeatureArray[this.instance].paintFeatures(gFeatureArray[this.instance].opt.json)
    }
  },
    ...
    for (var i = 0; i < keyShapes.length; i++) {
      ...
      _withSliderAndButton: function (sizeX, sizeY, sizeYKey) {
        var text =
          ...
          'onclick="gFeatureArray['+this.instance+'].exportFeaturesToImage();"/>' +
          '</div> ' +
          '</td>' +
          '</tr>' +
          '</table>' +
